...
//! root-level anchored popover ([`menu_overlay`](EcadApp::menu_overlay), stacked
//! by `build`). The open-menu slot lives in [`EcadApp::open_menu`] and is folded
...
//! |---|---|---|
//! | File ▸ Save | [`SAVE_KEY`] | [`EcadApp::save`] |
//! | File ▸ Revert to Saved | [`REVERT_KEY`] | [`EcadApp::revert_to_saved`] |
//! | File ▸ Libraries… / Tools ▸ Libraries… | [`LIBRARIES_TOGGLE_KEY`] | Libraries modal |
//! | Edit ▸ Undo | [`UNDO_KEY`] | [`EcadApp::undo`] |
//! | Edit ▸ Redo | [`REDO_KEY`] | [`EcadApp::redo`] |
...
/// discarding in-memory edits ([`EcadApp::revert_to_saved`]).
...
pub(crate) struct MenuDef {
    /// Lowercase token in `menubar:menu:{value}` and the [`EcadApp::open_menu`] slot.
    ...
    vec![
        ...
        MenuDef {
            ...
            rows: vec![
                ...
                dis("Rotate", Some("R")),
                Sep,
                dis("Copy", Some("Ctrl+C")),
                dis("Paste", Some("Ctrl+V")),
                Sep,
        ...
        MenuDef {
            value: "help",
            label: "Help",
            rows: vec![dis("Keymap", None), dis("About ecad", None)],
        },
    ]
}
