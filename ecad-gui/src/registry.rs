...
//!    (`$XDG_CONFIG_HOME/ecad/libraries`, falling back to
//!    `$HOME/.config/ecad/libraries`).
//! 2. [`resolve`] — the (re)load-time resolution step: parse the source, walk
//!    its `use` names through the registry, [`ecad_core::library::load_library`]
...
//!
//! # Registry file format (hand-rolled, `.ecad`-family line grammar)
//!
//! ```text
//! # ecad library registry: NAME <absolute path>
...
impl Registry {
    ...
    pub fn save(&self, path: &Path) -> Result<(), String> {
        ...
        let mut out = String::from("# ecad library registry: NAME <absolute path>\n");
...
/// A registered package directory that failed [`ecad_core::library::load_library`].
pub const W_LIB_LOAD: &str = "W_LIB_LOAD";
/// A part-name collision reported by [`ecad_core::library::union`] (first wins).
...
/// each time.
///
/// - Parse the source ([`ecad_core::text::parse`]) and take its `use` names in
///   source order ([`ecad_core::library::use_names`]). A source that does not
...
///
/// [`load_library`]: ecad_core::library::load_library
/// [`union`]: ecad_core::library::union
/// [`part_library`]: ecad_core::part::part_library
pub fn resolve(source: &str, registry: &Registry) -> (PartLib, Vec<LibNote>) {
    ...
    if let Ok(parsed) = ecad_core::text::parse(source) {
        for name in ecad_core::library::use_names(&parsed.source) {
            match registry.get(&name) {
                ...
                Some(dir) => match ecad_core::library::load_library(dir) {
    ...
    libs.push(("builtin".to_string(), ecad_core::part::part_library()));
    let (lib, collisions) = ecad_core::library::union(&libs);
...
pub enum RowStatus {
    ...
    Ok { parts: usize },
    /// The directory (or its `ecad.lib` manifest) does not exist.
    Missing,
...
/// [`ecad_core::library::load_library`] the resolver uses. Distinguishes a
...
pub fn row_status(dir: &Path) -> RowStatus {
    if !dir.join(ecad_core::library::MANIFEST_NAME).exists() {
        return RowStatus::Missing;
    }
    match ecad_core::library::load_library(dir) {
...
mod tests {
    ...
    impl Scratch {
        fn new(tag: &str) -> Scratch {
            let dir = std::env::temp_dir()
                .join(format!("ecad-registry-test-{tag}-{}", std::process::id()));
    ...
    fn resolve_no_use_is_toy_lib() {
        ...
        assert!(same_parts(&lib, &ecad_core::part::part_library()));
    ...
    fn resolve_broken_package_notes() {
        ...
        assert!(
            same_parts(&lib, &ecad_core::part::part_library()),
