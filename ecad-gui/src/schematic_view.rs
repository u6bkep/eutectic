...
//! tags, and text labels, matching [`schematic_svg`](ecad_core::schematic_svg)'s
...
//! **stroked glyph polylines** via [`ecad_core::font::text_strokes`] (the same public
...
use ecad_core::annotate;
use ecad_core::coord::{MM, Nm, Point};
use ecad_core::doc::{Doc, Orient};
use ecad_core::font::{Justify, text_strokes};
use ecad_core::id::{EntityId, NetId};
use ecad_core::part::{PartDef, PartLib};
use ecad_core::schematic::{PinSide, PinSlot, Placement, Wire, pin_slots, symbol_extent};
...
fn layout_symbols(layout: &ecad_core::schematic::SchematicLayout) -> Vec<(String, Orient)> {
    ...
    use ecad_core::schematic::LayoutNode;
...
fn box_color() -> Color {
    Color::srgb_token("ecad.schematic.ink", 0xd8, 0xd8, 0xd8, 0xff)
}

/// Wires — a green trace, matching the SVG's `#0a0` wire stroke intent.
fn wire_color() -> Color {
    Color::srgb_token("ecad.schematic.wire", 0x2e, 0xa0, 0x43, 0xff)
}

/// Net tags — a muted cyan so net labels read distinct from the ink.
fn tag_color() -> Color {
    Color::srgb_token("ecad.schematic.tag", 0x6f, 0xb7, 0xc9, 0xff)
}
