...
use ecad_core::elaborate::{psu_module, GenDirective, Source};
...
fn main() {
    ...
    h.commit(Transaction::one(Command::SetSource(uart)), &lib, "uart").unwrap();
    print!("{}", render(h.doc()));
    ...
    let contention: Source = vec![
        GenDirective::Instance { path: "reg1".into(), part: "LDO".into() },
        GenDirective::Instance { path: "reg2".into(), part: "LDO".into() },
        GenDirective::ConnectPins {
            net: "VBUS".into(),
            pins: vec![("reg1".into(), "VOUT".into()), ("reg2".into(), "VOUT".into())],
        },
    ];
    let mut h2 = History::new(Default::default());
    h2.commit(Transaction::one(Command::SetSource(contention)), &lib, "contend").unwrap();
    ...
    h3.commit(Transaction::one(Command::SetSource(psu_module(2))), &lib, "psu2").unwrap();
    ...
    h3.commit(Transaction::one(Command::SetSource(src)), &lib, "spare").unwrap();
    eng.query(h3.doc(), &lib, Key::Erc);
    println!(
    ...
    h4.commit(Transaction::one(Command::SetSource(psu_module(3))), &lib, "psu3").unwrap();
    h4.commit(
    ...
    h4.commit(Transaction::one(Command::SetSource(psu_module(5))), &lib, "psu5").unwrap();
    print!("{}", render(h4.doc()));
    ...
    h4.commit(Transaction::one(Command::SetSource(psu_module(1))), &lib, "psu1").unwrap();
    print!("{}", render(h4.doc()));
    ...
    h4.undo();
    println!("  after undo:     {}  (back to the 5-cap version)", h4.doc().components.len());
}
