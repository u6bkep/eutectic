...
use ecad_core::elaborate::{psu_module, GenDirective};
...
fn main() {
    ...
    let mut h = History::new(Default::default());
    h.commit(Transaction::one(Command::SetSource(psu_module(2))), &lib, "psu2").unwrap();
    h.commit(Transaction::one(Command::Nudge(dec0.clone(), Point::mm(10, 0))), &lib, "noop-nudge")
        .unwrap();
    println!("nudged dec[0] to (10mm,0) — exactly its default:");
    print!("{}", render(h.doc()));
    println!("  override map now holds {} entries (the dead hint was collected).", h.doc().overrides.len());
    ...
    let mut h = History::new(Default::default());
    h.commit(Transaction::one(Command::SetSource(psu_module(2))), &lib, "psu2").unwrap();
    h.commit(Transaction::one(Command::Pin(dec0.clone(), Point::mm(5, 5))), &lib, "pin").unwrap();
    let mut src = psu_module(2);
    src.push(GenDirective::Fix { path: "psu.dec[0]".into(), pos: Point::mm(8, 8) });
