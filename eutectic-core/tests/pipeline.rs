//! End-to-end pipeline integration tests (moved verbatim from src/lib.rs).

use eutectic_core::command::{Command, Resolution, Transaction, suggested_resolutions};
use eutectic_core::doc::{DecayReason, Doc, MM, Nm, Point, Provenance};
use eutectic_core::elaborate::{GenDirective, Source, board_rect, psu_module};
use eutectic_core::geom::Shape2D;
use eutectic_core::geom::kernel::{DEFAULT_CIRCLE_SEGS, difference, shape_to_region};
...
use eutectic_core::route::{Trace, Via, Violation};
use eutectic_core::solve::{Constraint, PLACE_TOL, Problem, dist, solve};
...
fn padless_lib() -> eutectic_core::part::PartLib {
    use eutectic_core::part::{PartDef, PinDef, PinRole};
    ...
    let mut lib = eutectic_core::part::PartLib::new();
    lib.insert(
        "LDO".into(),
        part(
            "LDO",
...
fn transaction_is_atomic_on_error() {
    ...
    .unwrap();
    let before = eutectic_core::project::render(h.doc());
    ...
    assert!(r.is_err());
    assert_eq!(before, eutectic_core::project::render(h.doc()));
}
...
fn dup_power_lib() -> eutectic_core::part::PartLib {
    use eutectic_core::part::{PartDef, PinDef, PinRole};
...
fn connect_to_unknown_pin_is_a_hard_error() {
    ...
    let text = eutectic_core::diagnostic::render(&err);
...
fn elaboration_collects_all_independent_faults() {
    ...
    let text = eutectic_core::diagnostic::render(&errs);
    assert!(
...
fn pad_lib() -> eutectic_core::part::PartLib {
    use eutectic_core::geom::Shape2D;
    use eutectic_core::part::{PadCopper, PadGeo, PadLayers, PartDef, PinDef, PinRole};
    let pin = PinDef {
        name: "1".into(),
        number: "1".into(),
        role: PinRole::Passive,
...
fn placement_avoids_courtyard_overlap() {
    ...
    let sep = 2 * (250_000 + eutectic_core::part::COURTYARD_MARGIN);
...
fn bar_lib() -> eutectic_core::part::PartLib {
    use eutectic_core::geom::Shape2D;
    use eutectic_core::part::{PadCopper, PadGeo, PadLayers, PartDef, PinDef, PinRole};
    let pad = |w, h| PinDef {
        name: "1".into(),
...
fn near_to_pin_pulls_component_onto_rotated_pin() {
    use eutectic_core::doc::Orient;
    use eutectic_core::part::pin_world;
...
fn resolve_validates_against_report_and_is_atomic() {
    ...
    assert_eq!(
        before,
        eutectic_core::project::render(h.doc()),
