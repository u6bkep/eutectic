...
//! entirely through the eutectic-core framework and rebuilt as a real **4-layer** design.
...
use eutectic_core::diagnostic::render;
use eutectic_core::doc::{MM, Point};
use eutectic_core::elaborate::{GenDirective as G, RegionDecl, Source};
use eutectic_core::export::{excellon_drill, fab_svg_set, gerber_set, netlist, placement_csv, svg};
...
use eutectic_core::text::{serialize, serialize_schematic_block};
...
/// hand-rolled `build_lib()` now lives as data in `poc/parts/eutectic.lib`.
...
impl Builder {
    ...
    fn text(&mut self, string: &str, x: i64, y: i64, h_mm_x10: i64, layer: &str) {
        self.s.push(G::Text {
            ...
            layer: layer.into(),
            orient: eutectic_core::doc::Orient::IDENTITY,
        });
    }
...
fn build_source() -> Source {
    ...
    // the library package its parts come from, so the serialized board.eut opens
...
fn sym(path: &str) -> LayoutNode {
    LayoutNode::Symbol(Symbol {
        path: path.into(),
        rot: eutectic_core::doc::Orient::IDENTITY,
        dx: 0,
        dy: 0,
    })
}
...
fn main() {
    ...
    let unrouted_before = before
        .iter()
        .filter(|v| matches!(v, eutectic_core::route::Violation::Unrouted { .. }))
        .count();
    println!(
    ...
    let (clr, mw, un): (Vec<_>, Vec<_>, Vec<_>) = {
        use eutectic_core::route::Violation::*;
        let mut clr = vec![];
        let mut mw = vec![];
        let mut un = vec![];
        for v in &after {
    ...
    write("board.eut", &serialize(doc), &mut wrote);
