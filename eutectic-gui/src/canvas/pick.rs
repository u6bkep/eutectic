...
//!    [`FeatureOrigin`](eutectic_core::geom::FeatureOrigin) naming the source entity it
...
//!
//! `eutectic-core`'s [`world_features`](eutectic_core::route::world_features) stream is *the*
//! render producer, and each [`NetFeature`](eutectic_core::geom::NetFeature) now carries a
//! [`FeatureOrigin`](eutectic_core::geom::FeatureOrigin) naming the source entity it was
...
//! `Void`s, silk/text markings, [`Unattributed`](eutectic_core::geom::FeatureOrigin::Unattributed))
...
pub enum SemanticId {
    ...
    /// and `PinRef`'s contract (`eutectic_core::doc::PinRef`).
...
mod tests {
    ...
    fn fixture() -> (Vec<Candidate>, eutectic_core::part::PartLib) {
        let d = board_domain();
    ...
    /// Manual perf probe (`cargo test -p eutectic-gui -- --ignored pick_resolve_perf
