...
/// The real 4-layer multiprobe board (poc/out/board.eut) parses, elaborates, and
...
fn poc_multiprobe_board_projects() {
    ...
    let doc = d
        .doc
        .as_ref()
        .expect("poc/out/board.eut elaborates with the poc library");
...
fn bounds_exclude_fab_datum_and_track_svg() {
    for name in ["board", "poc"] {
        ...
        let svg = eutectic_core::export::svg(doc, &d.lib).unwrap();
