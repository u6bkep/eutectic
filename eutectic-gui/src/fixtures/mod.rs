//! Canned [`EutecticApp`] states for the headless review loop.
//!
...
//! document loaded (from a tiny inline `.eut` source), and a parse-error
//! state.
//!
...
use crate::app::{DomainState, EutecticApp};

// ---------------------------------------------------------------------------
// The real 4-layer multiprobe board, loaded from `poc/out/board.eut` with the
...
/// The real multiprobe board loaded from `poc/out/board.eut` with the `poc`
/// library package loaded from `poc/parts` (the same manifest-driven
/// [`eutectic_core::library::load_library`] path the `poc_multiprobe` example uses —
...
pub fn poc_board_domain() -> DomainState {
    ...
    let lib = eutectic_core::library::load_library(&parts)
        .unwrap_or_else(|e| panic!("load library package from {}: {e}", parts.display()));
    let path = poc_dir().join("out/board.eut");
    let source =
        std::fs::read_to_string(&path).unwrap_or_else(|e| panic!("read {}: {e}", path.display()));
    DomainState::from_source_with(source, Some("board.eut".to_string()), lib, |_| Vec::new())
}
...
pub fn all() -> Vec<(&'static str, EutecticApp)> {
    vec![
