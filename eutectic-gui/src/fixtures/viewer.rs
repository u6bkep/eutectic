...
/// A tiny but complete `.eut` document: two parts, two nets, and a board
...
pub fn no_document() -> EutecticApp {
    EutecticApp::new(DomainState::empty())
}

/// A document loaded from [`SAMPLE_ECAD`].
pub fn document_loaded() -> EutecticApp {
    EutecticApp::new(DomainState::from_source(
        SAMPLE_ECAD.to_string(),
        Some("sample.eut".to_string()),
    ))
}

/// A parse/elaborate-error state loaded from [`BROKEN_ECAD`].
pub fn parse_error() -> EutecticApp {
    EutecticApp::new(DomainState::from_source(
        BROKEN_ECAD.to_string(),
        Some("broken.eut".to_string()),
    ))
}
...
pub fn board() -> EutecticApp {
    EutecticApp::new(board_domain())
}
...
pub fn menubar_open() -> EutecticApp {
    let app = board();
