...
/// The command palette owns raw editor keys as well as registered chords, so
/// Delete/R cannot mutate the selected board component behind its modal.
#[test]
fn runtime_delete_and_r_are_suppressed_while_palette_is_open() {
    let mut app = edit_app();
    let c1 = EntityId::new("C1");
    app.domain
        .selection
...
/// Help dialogs consume every runtime key event before document dispatch, so
/// the selected board component survives both Delete and R unchanged.
#[test]
fn runtime_delete_and_r_are_suppressed_while_help_is_open() {
    let mut app = edit_app();
    let c1 = EntityId::new("C1");
    app.domain
        .selection
        ...
        .select_only(SemanticId::Part(c1.clone()));
    app.chrome_dialog
        .set(Some(crate::chrome::dialogs::ChromeDialog::Keymap));
    let mut runtime = RunnerCore::new();
    runtime.set_hotkeys(app.hotkeys());
    ...
    let component = &app.domain.doc.as_ref().unwrap().components[&c1];
    assert_eq!(component.orient.to_deg(), 0);
    assert!(app.chrome_dialog.get().is_some());
    assert_eq!(app.undo_depths(), (0, 0));
}

/// Bare Delete/R remain board-only when generated by the runtime: focusing the
/// schematic pane leaves the selected board component untouched.
#[test]
fn runtime_delete_and_r_are_inert_with_schematic_pane_focused() {
    let mut app = edit_app();
    let c1 = EntityId::new("C1");
    app.domain
        .selection
