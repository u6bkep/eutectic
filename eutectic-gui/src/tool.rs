...
//! moment is the focused pane's kind's entry (`EutecticApp::tool_for` /
//! `EutecticApp::set_tool` / `EutecticApp::live_tool`). A kind with no entry defaults to
...
use eutectic_core::geom::kernel::Region;
use eutectic_core::geom::{Path, Seg, Shape2D};
