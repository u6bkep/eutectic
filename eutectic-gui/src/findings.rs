...
//!
//! `eutectic-core` surfaces findings as **semantic** refs, never board mm:
//!
...
fn violation_refs(v: &eutectic_core::route::Violation) -> (&'static str, Vec<SemanticId>) {
...
/// [`halo_point`]. Mirrors `EutecticApp::candidate_net` (built in `panes`; kept local so
