...
//! twin of `eutectic-core`'s SVG backend (`export/svg.rs`): it walks the same unified
//! [`world_features`](eutectic_core::route::world_features) stream, bins each physical
...
use eutectic_core::geom::{Extent, NetFeature, Role, Shape2D, Stackup, ZRange};
...
/// [`Layer`](eutectic_core::route::Layer) ordinal: this enumerates *every* visual layer
...
impl Canvas {
    ...
    fn layer_order(&self, su: &Stackup) -> Vec<LayerId> {
        let mut slabs: Vec<&eutectic_core::geom::Slab> = su.slabs.iter().collect();
...
/// cheap comparisons. Owned per pane by the app (`EutecticApp::grid_caches`) — the
...
fn finding_error_color() -> Color {
    Color::srgb_token("eutectic.finding.error", 0xff, 0x45, 0x45, 0xff)
}

/// The warning-finding marker colour — a warm amber ring (matches the DRC-warn chip).
fn finding_warning_color() -> Color {
    Color::srgb_token("eutectic.finding.warning", 0xf5, 0xc0, 0x24, 0xff)
}
...
fn overlay_hover_color() -> Color {
    Color::srgb_token("eutectic.overlay.hover", 0x22, 0xd3, 0xee, 0x88)
}

/// The measure-line accent — a warm amber, distinct from the selection cyan.
fn measure_color() -> Color {
    Color::srgb_token("eutectic.overlay.measure", 0xf5, 0xa5, 0x24, 0xff)
}
...
fn ghost_color() -> Color {
    Color::srgb_token("eutectic.overlay.ghost", 0xb8, 0x8c, 0xff, 0xdd)
}
...
fn ratsnest_color() -> Color {
    Color::srgb_token("eutectic.overlay.ratsnest", 0xe8, 0xe4, 0xa0, 0xbb)
}
...
/// both.
pub(crate) fn slab_of_z<'a>(su: &'a Stackup, z: &ZRange) -> Option<&'a eutectic_core::geom::Slab> {
...
// each carries a stable `eutectic.layer.*` token name via [`Color::srgb_token`]. The
...
fn outline_color() -> Color {
    Color::srgb_token("eutectic.layer.edge", 0xea, 0xb3, 0x08, 0xff)
}

/// Drill / hole colour: near-black, so a plated barrel reads as a punched void.
fn hole_color() -> Color {
    Color::srgb_token("eutectic.layer.drill", 0x10, 0x10, 0x10, 0xff)
}
...
fn grid_dot_color() -> Color {
    Color::srgb_token("eutectic.grid.dot", 0x28, 0x28, 0x2e, 0xff)
}
...
fn grid_axis_color() -> Color {
    Color::srgb_token("eutectic.grid.axis", 0x3b, 0x82, 0xf6, 0x55)
}
...
fn copper_color_top() -> Color {
    Color::srgb_token("eutectic.layer.cu.top", 0xd6, 0x3a, 0x3a, 0xff)
}
fn copper_color_bottom() -> Color {
    Color::srgb_token("eutectic.layer.cu.bottom", 0x3a, 0x7a, 0xd6, 0xff)
}
fn copper_color_inner() -> Color {
    Color::srgb_token("eutectic.layer.cu.inner", 0x3a, 0xb0, 0x55, 0xff)
}
