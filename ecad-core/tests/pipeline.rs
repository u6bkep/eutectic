...
use ecad_core::command::{Command, Resolution, Transaction, suggested_resolutions};
...
fn transaction_is_atomic_on_error() {
    ...
    assert!(r.is_err());
    assert_eq!(before, ecad_core::project::render(h.doc()));
}
...
fn dup_power_lib() -> ecad_core::part::PartLib {
...
fn connect_to_unknown_pin_is_a_hard_error() {
    ...
    let text = ecad_core::diagnostic::render(&err);
...
fn elaboration_collects_all_independent_faults() {
    ...
    let text = ecad_core::diagnostic::render(&errs);
    assert!(
...
fn pad_lib() -> ecad_core::part::PartLib {
    use ecad_core::geom::Shape2D;
    use ecad_core::part::{PadCopper, PadGeo, PadLayers, PartDef, PinDef, PinRole};
    let pin = PinDef {
        name: "1".into(),
        number: "1".into(),
...
fn placement_avoids_courtyard_overlap() {
    ...
    let sep = 2 * (250_000 + ecad_core::part::COURTYARD_MARGIN);
...
fn bar_lib() -> ecad_core::part::PartLib {
    use ecad_core::geom::Shape2D;
    use ecad_core::part::{PadCopper, PadGeo, PadLayers, PartDef, PinDef, PinRole};
    let pad = |w, h| PinDef {
        name: "1".into(),
...
fn resolve_validates_against_report_and_is_atomic() {
    ...
    assert_eq!(
        before,
        ecad_core::project::render(h.doc()),
